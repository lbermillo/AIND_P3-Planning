...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # 1. Determine what actions to add
        action_nodes = []

        for action in self.all_actions:
            # Create PgNode_a object for every action
            action_node = PgNode_a(action)

            # See if the action_node has prenodes that are a subset of the previous S level
            # and append it to the the list if satisfies this requirement
            if action_node.prenodes.issubset(self.s_levels[level]):
                action_nodes.append(action_node)

                # 2. Once action_node is added, it MUST be connected to
                #    the state_node instances in the appropriate s_level set.
    ...
    def add_literal_level(self, level):
        ...
        for action_nodes in self.a_levels[level - 1]:
            # 2. Connect all of the new S nodes as children of all the A nodes that could produce them,
            #    and likewise add the A nodes to the parent sets of the S nodes
            for new_state_nodes in action_nodes.effnodes:
                state_nodes.add(new_state_nodes)
                new_state_nodes.parents.add(action_nodes)
                action_nodes.children.add(new_state_nodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Go through each positive effect in node_a1
        for a1_effect in node_a1.action.effect_add:
            # Return True if any of node_a2's actions negates node_a1's by checking
            # membership of node_a1's positive effect in node_a2's negative effects
            if a1_effect in node_a2.action.effect_rem:
                return True

        # Go through each positive effect in node_a2
        for a2_effect in node_a2.action.effect_add:
            # Return True if any of node_a1's actions negates node_a2's by checking
            # membership of node_a2's positive effect in node_a1's negative effects
            if a2_effect in node_a1.action.effect_rem:
                return True

        # Return False if no negation occurs
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if one node's added effects interferes with the other node's negative preconditions

        # Loop through added effects from node_a1's actions
        # and check if it's contained in node_a2's negative preconditions
        for a1_effect in node_a1.action.effect_add:
            if a1_effect in node_a2.action.precond_neg:
                # Return True if we get a hit
                return True
        ...
        # Now check if one node's removed effects interferes with the other node's positive preconditions

        for a1_effect in node_a1.action.effect_rem:
            if a1_effect in node_a2.action.precond_pos:
                return True

        for a2_effect in node_a2.action.effect_rem:
            if a2_effect in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Loop through a1 and a2 preconditions and check if the precondition of a1's actions is the mutex of a2
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Loop through each node's previous actions and check if each possible
        # pair of actions that could achieve the two literals is mutually exclusive
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        # For each goal, find the first level where the goal appears
        for goal in self.problem.goal:
            # Initialize the first level
            level = 0
            # Iterate through every state in the current level
            for state in self.s_levels[level]:
                # if goal matches a state symbol, add current level to level_sum and break the loop
